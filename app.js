...
function computeEffectiveness(retoquesSrc, retiradasSrc){
  ...
  const retiradaToRetoque = {}; // retirada.id -> retoque object (o null)

  Object.entries(retiradasPorMatriz).forEach(([matriz, retiradasDeMatriz]) => {
    const toques = (retoquesPorMatriz[matriz] || []).slice().sort((a,b) => (a.fecha||"").localeCompare(b.fecha||""));
    // ordena las retiradas de esa matriz por fecha para asignar de forma consistente
    const retiradasOrdenadas = retiradasDeMatriz.slice().sort((a,b) => (a.fecha||"").localeCompare(b.fecha||""));
  ...
  // Nº de veces que cada matriz ha sido retirada (dentro del rango filtrado)
  const vecesPorMatriz = {};
  retiradasList.forEach(r => { vecesPorMatriz[r.referencia] = (vecesPorMatriz[r.referencia]||0) + 1; });

  return { result, fallidoIds, retiradaToRetoque, vecesPorMatriz };
}
  ...
  tbodyDet.innerHTML = activas.map(r => {
    ...
    return `
      <tr>
        ...
        <td>${r.referencia}</td>
        <td>${r.motivo || "—"}</td>
        <td>${vecesPorMatriz[r.referencia] || 1}</td>
...
document.getElementById("btn-export-historico").addEventListener("click", () => {
  ...
  const detalleRows = activas.map(r => {
    ...
    return {
      ...
      "Referencia": r.referencia,
      "Motivo": r.motivo || "",
      "Nº veces retirada": vecesPorMatriz[r.referencia] || 1,
...
setupDropzoneFile("dropzone-pedidos", "file-pedidos", async (file) => {
  ...
  try {
    ...
    const idxUtil = colIndexByKeyword(header, "UTIL") >= 0
      ? colIndexByKeyword(header, "UTIL")
      : colIndexByKeyword(header, "MATRIZ");

    if (idxFecha < 0 || idxUtil < 0) throw new Error("No se encontraron las columnas necesarias");

    const refToFechas = {};
    for (let i = headerIdx + 1; i < rows.length; i++){
      const r = rows[i];
      if (!r) continue;
      const util = r[idxUtil];
      const fechaRaw = r[idxFecha];
      if (util && fechaRaw){
        const fecha = excelDateToJSDate(fechaRaw);
        if (fecha){
          const ref = normMatriz(String(util).split("-")[0]);
...
setupDropzoneFile("dropzone-matrices", "file-matrices", async (file) => {
  ...
  try {
    const rows = await loadWorkbookRows(file);
    const headerIdx = findHeaderRow(rows, ["MATRIZ"]);
    if (headerIdx < 0) throw new Error("No se encontró la columna Matriz");
    const header = rows[headerIdx];
    const idxMatriz = colIndexByKeyword(header, "MATRIZ");
    const idxObs = colIndexByKeyword(header, "OBSERVA");
    if (idxMatriz < 0) throw new Error("No se encontró la columna Matriz");
...
document.getElementById("btn-generar-orden").addEventListener("click", () => {
  ...
  const filas = ordenMatricesData.map(m => {
  ...
  renderOrdenTrabajo(ordenFinal);
});

function renderOrdenTrabajo(ordenFinal){
  document.getElementById("card-orden-resultado").style.display = "block";
  document.getElementById("orden-fecha-title").textContent = fmtDateJS(new Date());
  document.getElementById("orden-count").textContent =
    `${ordenFinal.length} matrices · ${ordenFinal.filter(f => f.vencida).length} vencidas (muy urgentes)`;
...
document.getElementById("btn-export-orden").addEventListener("click", () => {
  ...
  const rows = ULTIMO_ORDEN.map(f => ({
    "Orden": f.orden,
    "Matriz": f.referencia,
    "Punto": f.punto || "",
    "Fecha entrega": f.fechaEntrega ? fmtDateJS(f.fechaEntrega) : "",
    "Estado": f.vencida ? "VENCIDA - MUY URGENTE" : (f.fechaEntrega ? "Con pedido" : "Sin pedido"),
    "Observación": f.observacion || ""
  }));
  downloadWorkbook([{ name: "Orden de trabajo", rows }], `orden_trabajo_matriceria_${new Date().toISOString().slice(0,10)}.xlsx`);
